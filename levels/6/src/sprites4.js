...
var itemTypes = [
    { img: 'src/assets/La_Sainte-Chapelle__Paris-removebg-preview.png', block: false },
];
...
var itemTypes1 = [
    { img1: 'src/assets/tour.png', block: false },
];
...
var itemTypes2 = [
    { img2: 'src/assets/notre dame de paris.png', block: false },
...
var itemTypes3 = [
     { img3: 'src/assets/arc de triomphe.png', block: false },
...
var itemTypes4 = [
    { img4: 'src/assets/—Pngtree—louvre glass pyramid hand drawn_3877931.png', block: false },
];
...
var itemTypes5 = [
    { img5: 'src/assets/Château de Chambord.png', block: false },
];
...
initSprites6 = function () {

    addItems6();
     var screen = $('screen');
    for (var i = 0; i < mapSprites6.length; i++) {


        var sprite6 = mapSprites6[i];
        var itemType6 = itemTypes6[sprite6.type];
        var img6 = document.createElement('img');
        img6.src = itemType6.img6;
        img6.style.display = "none";
        img6.style.position = "absolute";
        img6.style.overflow = "hidden";
        //img6.style.marginTop=28
        sprite6.state = 0;
        sprite6.numOfStates = itemType6.numOfStates;
        ...
        sprite6.block = itemType6.block;
        sprite6.img6 = img6;
        //spritePosition[sprite6.y][sprite6.x] = sprite6;
        sprites6.push(sprite6);
        screen.appendChild(img6);
...
var itemTypes6 = [
    { img6: 'src/assets/ve.png', block: false },
];
...
renderSprites6 = function () {
    for (var i = 0; i < sprites6.length; i++) {

        var sprite6 = sprites6[i];
        var dx = sprite6.x - player.x;
        var dy = sprite6.y - player.y;
        var distance = Math.sqrt(dx * dx + dy * dy);
            var angle = Math.atan2(dy, dx) - player.rotation;
            if (angle < -Math.PI) angle += Math.PI * 2;
            if (angle >= Math.PI) angle -= Math.PI * 2;
            if ((angle > -Math.PI) && (angle < Math.PI)) {
                ...
                var img6 = sprite6.img6;
                var size = viewDist / (Math.cos(angle) * distance);
                var x = Math.tan(angle) * viewDist;
                var prevStyle = sprite6.prevStyle;

                if (size != prevStyle.height) {
                    img6.style.height = size + 'px';
                    prevStyle.height = size;
                }
                // times the total number of states
                if ((size * sprites6.numOfStates) != prevStyle.width) {
                    img6.style.width = (size * sprite6.numOfStates) + 'px';
                    prevStyle.width = (size * sprite6.numOfStates);
                }
                if (((screenHeight - size) / 2) != prevStyle.top) {
                    img6.style.top = ((screenHeight - size) / 2) + 'px';
                    prevStyle.top = ((screenHeight - size) / 2);
                }
                if ((screenWidth / 2 + x - size / 2 - size * sprite6.state) != prevStyle.left) {
                    img6.style.left = (screenWidth / 2 + x - size / 2 - size * sprite6.state) + 'px';
                    prevStyle.left = (screenWidth / 2 + x - size / 2 - size * sprite6.state);
                }
                if (("brightness(" + (100 - 15 * distance) + "%)") != prevStyle.filter) {
                    img6.style.filter = ("brightness(" + (100 - 15 * distance) + "%)");
                    prevStyle.filter = ("brightness(" + (100 - 15 * distance) + "%)");
                }
                if (size >> 0 != prevStyle.zIndex) {
                    img6.style.zIndex = size >> 0;
                    prevStyle.zIndex = size >> 0;
                }
                if ('block' != prevStyle.display) {
                ...
                if (('rect(0, ' + (size * (sprite6.state + 1)) + ', ' + size + ', ' + (size * (sprite6.state)) + ')') != prevStyle.clip) {
                    img6.style.clip = ('rect(0, ' + (size * (sprite6.state + 1)) + ', ' + size + ', ' + (size * (sprite6.state)) + ')');
                    prevStyle.clip = ('rect(0, ' + (size * (sprite6.state + 1)) + ', ' + size + ', ' + (size * (sprite6.state)) + ')');
                }
            }
